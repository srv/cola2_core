...
def get_cola2_lib_version() -> str:
    ...
    filename = '/usr/local/share/cola2_lib/COLA2_LIBConfigVersion.cmake'
...
def get_packages(folder: str) -> List[Package]:
    ...
    for f in subfolders:
        ...
        try:
            repo = Repo(f)
            # only for iqua repos
            if "iquarobotics" in repo.remotes.origin.url:
                try:
                    link = repo.remotes.origin.url
                    name = link.split('/')[-1].split('.git')[0]
                    packages.append(
                        Package(
                            name=name,
...
def show_nice_table(cola2_lib_version: str, packages: List[Package]) -> str:
    # nice table
    toprint = list()
    toprint.append(('Repository', 'C', 'U', 'Branch', 'Date', 'Commit'))
...
def get_version() -> str:
    ...
    packages = get_packages(folder=catkin_src)
...
if __name__ == '__main__':
    ...
    pub.publish(String(get_version()))
