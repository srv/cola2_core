...
void IAUVROSController::checkDiagnostics(const ros::TimerEvent& event)
{
  ...
  {
    diagnostic_.setLevel(diagnostic_msgs::DiagnosticStatus::WARN);
    if (diagnostic_.getTimesNotOK() > 10)
    {
      diagnostic_.setLevel(diagnostic_msgs::DiagnosticStatus::ERROR);
    }
  }
  else
  {
  ...
  {
    diagnostic_.setLevel(diagnostic_msgs::DiagnosticStatus::WARN, "pose_controller_disabled");
  }
  if (!auv_controller_->isVelocityControllerEnable())
  {
    diagnostic_.setLevel(diagnostic_msgs::DiagnosticStatus::WARN, "velocity_controller_disabled");
  }
  if (!auv_controller_->isThrusterAllocatorEnable())
  {
    diagnostic_.setLevel(diagnostic_msgs::DiagnosticStatus::WARN, "thruster_allocator_disabled");
  }
}
...
void IAUVROSController::timerCallback(const ros::TimerEvent& event)
{
  ...
  diagnostic_.increaseFrequencyCounter();
  ...
  publishFinSetpoint(auv_controller_->getFinSetpoints(), now);
}
...
void IAUVROSController::publishFinSetpoint(const Eigen::VectorXd setpoint, const ros::Time now)
{
  ...
  // Publish message
  pub_fins_setpoint_.publish(output);
}
